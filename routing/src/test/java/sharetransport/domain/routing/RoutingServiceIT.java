...
public class RoutingServiceIT {
  ...
  private static Logger LOG = LoggerFactory.getLogger(RoutingServiceIT.class);
  ...
  public void whenRouteAllHopsExpectShortestPathIsCorrect() throws Throwable
  {
    try (Neo4jTestUnit dbUnit = Neo4jTestUnit.create(session,
        RoutingServiceIT.class.getResourceAsStream("/cypher/routing/4_hops_in_series.cypher"))) {
  ...
  {
    try (Neo4jTestUnit dbUnit = Neo4jTestUnit.create(session,
        RoutingServiceIT.class.getResourceAsStream("/cypher/routing/3_hops_by_two_origins_and_one_destination.cypher"))) {
